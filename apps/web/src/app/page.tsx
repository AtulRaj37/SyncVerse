...
export default function LandingPage() {
  ...
  const handleAuthAndCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!currentToken || authMode === 'LOGIN' || authMode === 'REGISTER') {
        ...
        const authRes = await fetch(`http://localhost:4000/api/auth/${endpoint}`, {
          method: "POST",
  ...
  const handleJoinRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!currentToken) {
        const authRes = await fetch("http://localhost:4000/api/auth/guest", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const res = await fetch(`http://localhost:4000/api/rooms/${endpoint}`);
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#060610] font-sans flex flex-col items-center justify-center relative">
      ...
      {/* Galaxy Background — window-level mouse events are listened to inside Galaxy.jsx */}
